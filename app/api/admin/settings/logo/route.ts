...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!(file.type in EXTENSIONS)) {
      return NextResponse.json({ error: "Logo harus berformat JPG, PNG, atau WebP" }, { status: 400 });
    }
    ...
    const path = `logos/logo-${Date.now()}.${EXTENSIONS[file.type]}`;
